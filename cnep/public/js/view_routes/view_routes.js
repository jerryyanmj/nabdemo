/**
 * This script is used for show data for view routes page
 */
...
( function () {
    ...
    function    show_route_information( source, time_range, source_data ) {
        ...
        console.log( "show_route_information(): source = " + source + ", time_range = " + time_range );
        ...
        if( source != "0" ) {
            ...
            $.each(
                ...
                function( i, key ) {
                    ...
                    // timerangetable_details[ 'Latency' ] = '100';
                    // timerangetable_details[ 'Loss' ]    = '100%';

                    // Rebuild data for charts
                    if( route_details != null ) {

                        // latencyData = createLatencyData( route_details, time_range );
                        // lossData    = createLossData( route_details, time_range );
                        ...
                        // load_time_range_table( timerangetable_details );
        ...
        // Redraw timerange table and charts
        //load_time_range_table( timerangetable_details );
    ...
    function    createLatencyData( route_details, time_range ) {
        ...
        for( var i = latencyChartStartHour; i <=  TIME_RANGE_UPPER_LIMIT; i++ ) {
            ...
            if( latencyDetails && latencyDetails != -1 ) {
                ...
                console.log( "latencyDetails.latency_moving_average = " + latencyDetails.latency_moving_average );
                console.log( "latencyDetails.latency_hourly_average = " + latencyDetails.latency_hourly_average );
    ...
    function    createBarChart( element, drawing_data,idx ) {
 	        ...
 	        return nv.addGraph(
   	            ...
   	            function() {
                    ...
                    console.log('element is '+element);
   	                ...
   	                if( element == ('#historical-rtt-chart-wrapper'+idx+' svg') ) {
    	
   	                    // For latency chart, display minimum range of 50ms, else max RTT value + 50ms
  	                    minY = drawing_data[ 0 ].values.length>0? drawing_data[ 0 ].values[ 0 ].y : 0;
   	                    maxY = drawing_data[ 0 ].values.length>0? drawing_data[ 0 ].values[ 0 ].y : 0;
    ...
    function    get_sla_sources_ajax() {
        ...
        $.ajax(
            {
                url:        '/cnep/ajax/view-routes-sources.json',
                dataType:   'json',
    ...
    function    get_source_data_ajax(source,timeRange) {
        ...
        $.ajax(
            {
                url:        '/cnep/ajax/view-routes-source-data/' + source + '/' + timeRange,
                dataType:   'json',
                error:      function ( error ) {
    ...
    ( function main() {
        ...
        $( document ).ready(
            ...
            function() {
                ...
                // Load current source, destination cities
                get_sla_sources_ajax()

                // Set function call for source change
                $( '#source' ).change(
